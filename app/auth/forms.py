...
Province_choice = [('北京', '北京'), ('天津', '天津'), ('上海', '上海'), ('天津', '天津'), ('重庆', '重庆'),
                   ('河北', '河北'), ('山东', '山东'), ('辽宁', '辽宁'), ('黑龙江', '黑龙江'), ('吉林', '吉林'),
                   ('甘肃', '甘肃'), ('青海', '青海'), ('河南', '河南'), ('江苏', '江苏'), ('湖北', '湖北'),
                   ('湖南', '湖南'), ('江西', '江西'), ('浙江', '浙江'), ('广东', '广东'), ('云南', '云南'),
                   ('福建', '福建'), ('台湾', '台湾'), ('海南', '海南'), ('山西', '山西'), ('四川', '四川'),
                   ('陕西', '陕西'), ('贵州', '贵州'), ('安徽', '安徽'), ('广西', '广西'), ('内蒙古', '内蒙古'),
                   ('西藏', '西藏'), ('新疆', '新疆'), ('宁夏', '宁夏'), ('澳门', '澳门'), ('香港', '香港')]


class LoginForm(FlaskForm):
    email = StringField('登录邮箱', validators=[DataRequired(), Length(1, 64), Email()])
...
class RegistrationForm(FlaskForm):
    ...
    username = StringField('用户名', validators=[
        DataRequired(), Length(1, 64), Regexp('^[A-Za-z][A-Za-z0-9_.]*$', 0, '支持英文和字母下划线。')])
    password = PasswordField('密码', validators=[
        DataRequired(), EqualTo('password2', message='两次密码不一致。')])
    password2 = PasswordField('确认密码', validators=[DataRequired()])
    name = StringField('姓名', validators=[
        DataRequired(), Length(1, 20), Regexp('^[a-zA-Z\u4e00-\u9fa5]*$', 0, '只能输入英文和中文。')])
    location = SelectField('地址', choices=Province_choice)
    about_me = TextAreaField('自我介绍')
    submit = SubmitField('立即注册')

    def validate_email(self, field):
        if MongoClient().blog.User.find_one({'email': field.data}):
...
class PasswordResetRequestForm(FlaskForm):
    ...
    def validate_email(self, email):
        if MongoClient().blog.User.find_one({'email': email.data}) is None:
...
class ChangeEmailForm(FlaskForm):
    email = StringField('新邮箱地址', validators=[DataRequired(), Length(1, 64), Email()])
    password = PasswordField('密码', validators=[DataRequired()])
    submit = SubmitField('修改')

    def validate_email(self, field):
        if MongoClient().blog.User.find_one({'email': field.data}) is not None:
