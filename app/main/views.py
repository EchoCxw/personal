...
from flask import render_template, abort, flash, request, make_response, redirect, url_for
from . import main
...
def edit_profile():
    ...
    if form.validate_on_submit():
        ...
        avatar.save('{}{}{}{}'.format(UPLOAD_FOLDER, current_user.username, '.', fname.rsplit('.', 1)[1]))
        current_user.avatar = '/static/avatar/{}{}{}'.format(current_user.username, '.', fname.rsplit('.', 1)[1])
        MongoClient().blog.User.update({'email': current_user.email}, {'$set': {'avatar': current_user.avatar}})
